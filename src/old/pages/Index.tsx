...
const Index = () => {
  const [userPhoto, setUserPhoto] = useState<{photo: string, gender: string} | null>(null);
  const [selectedMatch, setSelectedMatch] = useState<PotentialMatch | null>(null);
  const [selectedCelebrity, setSelectedCelebrity] = useState<Celebrity | null>(null);
  const [selectedCustomMatch, setSelectedCustomMatch] = useState<CustomMatch | null>(null);
  ...
  const [viewModalData, setViewModalData] = useState<{user1: any, user2: any} | null>(null);
  ...
  const handleViewMatch = (user1: {name: string, image: string}, user2: {name: string, image: string}) => {
    ...
    <div className="min-h-screen bg-gradient-subtle">
      <div className="container mx-auto py-4 sm:py-6 lg:py-8">
        ...
        <div className="text-center mb-16 animate-fade-in">
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-display font-light text-foreground mb-4 tracking-tight">
            Fuzed 
          </h1>
              ...
              <div className="animate-fade-in">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  <TabsList className="grid w-full grid-cols-3 mb-8 bg-card border border-border">
                    <TabsTrigger value="university" className="font-medium">University</TabsTrigger>
                    <TabsTrigger value="celebrity" className="font-medium">Celebrities</TabsTrigger>
                    <TabsTrigger value="custom" className="font-medium">Your Photos</TabsTrigger>
                  </TabsList>
            ...
            {showBabyGenerator && (
              <div className="animate-fade-in">
                <EnhancedBabyGenerator 
                  ...
                  matchPhoto={selectedMatch?.image || selectedCelebrity?.image || selectedCustomMatch?.image}
                  matchName={selectedMatch?.name || selectedCelebrity?.name || selectedCustomMatch?.name}
